...
import type { BudgetCode } from '@/modules/settings/components/FinanceSettings/budgetCode/BudgetCodeSection';
import type { FiscYearListDto } from '@/modules/core/types/fisc';

interface EditBudgetCodeModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const EditBudgetCodeModal: React.FC<EditBudgetCodeModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  budgetCode,
  ...
  useEffect(() => {
    if (budgetCode) {
      setFormData({
        ...
        fiscalYear: budgetCode.fiscalYear,
  ...
  if (!isOpen || !budgetCode) return null;
      ...
      >
        ...
        <div className="flex justify-between items-center border-b px-6 py-2 sticky top-0 bg-white z-10">
          <div className="flex items-center gap-2">
            <Code size={20} />
            <h2 className="text-lg font-bold text-gray-800">Edit Budget Code</h2>
          </div>
          <button
            onClick={onClose}
...
export default EditBudgetCodeModal;
