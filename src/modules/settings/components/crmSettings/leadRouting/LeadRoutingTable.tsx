...
import { Shield, MoreVertical, Edit, Trash2, ChevronLeft, ChevronRight, Settings } from "lucide-react";
import { Popover, PopoverTrigger, PopoverContent } from "@/shared/components/ui/popover";
import type { RoutingRule } from "@/modules/settings/components/crmSettings/leadRouting/LeadRoutingSection";
...
const LeadRoutingTable: React.FC<LeadRoutingTableProps> = ({ rules, onEdit, onDelete, onToggleActive, onManageConditions }) => {
  const [popoverOpen, setPopoverOpen] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState(1);

  const totalPages = Math.max(1, Math.ceil(rules.length / PAGE_SIZE));
  const paginated = rules.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
    ...
    >
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            {paginated.map((rule, index) => (
              ...
              >
                ...
                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-700">{rule.assignTo}</td>
                <td className="px-4 py-2 whitespace-nowrap">
                  <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${rule.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
                ...
                <td className="px-4 py-2 whitespace-nowrap">
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-orange-600 bg-orange-50 border border-orange-200 rounded-md hover:bg-orange-100 transition-colors"
                  >
                    <Settings className="w-3 h-3" /> Manage
                  </button>
                </td>
                <td className="px-4 py-2 whitespace-nowrap text-right">
                  <Popover open={popoverOpen === rule.id} onOpenChange={(o) => setPopoverOpen(o ? rule.id : null)}>
                    ...
                    <PopoverContent className="w-44 p-0" align="end">
                      <div className="py-1">
                        <button onClick={() => { onEdit(rule); setPopoverOpen(null); }} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 text-gray-700 flex items-center gap-2">
                          <Edit size={15} /> Edit
                        </button>
                        <button onClick={() => { onToggleActive(rule); setPopoverOpen(null); }} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 text-gray-700 flex items-center gap-2">
                          {rule.isActive ? 'Deactivate' : 'Activate'}
                        </button>
                        <button onClick={() => { onDelete(rule); setPopoverOpen(null); }} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
      ...
      <div className="bg-white px-6 py-3 flex items-center justify-between border-t border-gray-200">
        <p className="text-sm text-gray-700">
          Showing <span className="font-medium">{Math.min((currentPage - 1) * PAGE_SIZE + 1, rules.length)}</span> to{' '}
          <span className="font-medium">{Math.min(currentPage * PAGE_SIZE, rules.length)}</span> of{' '}
