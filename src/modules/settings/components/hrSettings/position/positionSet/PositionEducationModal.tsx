...
// Predefined education level options
...
const PositionEducationModal: React.FC<PositionEducationModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      // ✅ Use educationLevel (string), not educationLevelId
      setSelectedEducationLevel(editingEducation?.educationLevel || "");
  ...
  const getEducationIcon = (levelValue: string) => {
    const IconComponent = educationIcons[levelValue] || educationIcons.default;
  ...
  // Create enum object for EnumSelect
  const educationLevelEnum = EDUCATION_LEVELS.reduce(
      (acc, curr) => ({
        ...acc,
        [curr.id]: curr.name,
      }),
