// src/pages/finance/ifrs/IFRS15.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { motion } from 'framer-motion';
import {
    DollarSign, Search, RefreshCw, Eye, Download, Printer,
    ...
    Users, Building2, CheckCircle, Clock, Package,
    Receipt, ShoppingCart, BadgeCheck
} from 'lucide-react';
...
const IFRS15: React.FC = () => {
    ...
    const {
        ...
        handleExport,
        handleRefresh,
    } = useReportExport('ifrs15');

    const ITEMS_PER_PAGE = 10;

    // Mock data - These would come from API in production
    const revenueData: RevenueData[] = [
        { category: 'Products', amount: 2500000, percentage: 55.6, growth: 12.3 },
        { category: 'Services', amount: 1500000, percentage: 33.3, growth: 8.7 },
        { category: 'Licenses', amount: 500000, percentage: 11.1, growth: 5.2 },
    ];

    const performanceObligations: PerformanceObligation[] = [
        { status: 'Satisfied', count: 124, percentage: 79.5, color: 'bg-green-500' },
        { status: 'Unsatisfied', count: 32, percentage: 20.5, color: 'bg-yellow-500' },
    ];

    const transactionPriceData: TransactionPrice[] = [
        { component: 'Total Transaction Price', amount: 4500000, description: 'Total consideration from contracts' },
        { component: 'Variable Consideration', amount: 320000, description: 'Performance bonuses and rebates' },
        { component: 'Non-cash Consideration', amount: 150000, description: 'Non-monetary consideration' },
    ];
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            console.error('Error fetching IFRS 15 reports:', error);
            showToast.error('Failed to load IFRS 15 reports');
        } finally {
            setLoading(false);
    ...
    const handleGenerate = async () => {
        try {
            ...
            showToast.success('IFRS 15 report generated successfully');
            await fetchData();
        ...
        >
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">IFRS 15 - Revenue Recognition</h1>
                        <p className="text-sm text-gray-500">Revenue from contracts with customers - performance obligations and transaction price</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <Card className="bg-gradient-to-r from-blue-50 to-blue-100 border-blue-200">
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-blue-700 font-medium">Positive Metrics</p>
                                <p className="text-2xl font-bold text-blue-900">{stats.positiveMetrics}</p>
            ...
            {/* IFRS 15 Specific Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="border-l-4 border-l-blue-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Performance Obligations</h4>
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Total Obligations</span>
                                <span className="font-semibold text-gray-900">
                                    {performanceObligations.reduce((sum, p) => sum + p.count, 0)}
                                </span>
                            </div>
                            {performanceObligations.map((item, idx) => (
                                <div key={idx}>
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600">{item.status}</span>
                                        <span className={`font-semibold ${idx === 0 ? 'text-green-600' : 'text-yellow-600'}`}>
                ...
                <Card className="border-l-4 border-l-green-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Transaction Price</h4>
                        <div className="space-y-3">
                            {transactionPriceData.map((item, idx) => (
                                <div key={idx}>
                                    <div className="flex justify-between items-center">
                                        <div>
                                        ...
                                        <span className={`font-semibold ${idx === 0 ? 'text-blue-600' : 'text-gray-700'}`}>
                                            {formatCurrency(item.amount)}
                                        </span>
                                    </div>
                                    {idx < transactionPriceData.length - 1 && <div className="border-t border-gray-100 my-2" />}
                                </div>
                            ))}
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total Consideration</span>
                                    <span className="font-bold text-green-600">
                                        {formatCurrency(transactionPriceData.reduce((sum, item) => sum + item.amount, 0))}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <Card className="border-l-4 border-l-orange-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Revenue by Category</h4>
                        <div className="space-y-3">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-wrap gap-4">
                <div className="flex-1 min-w-[200px] relative">
                    ...
                    <Input
                        placeholder="Search IFRS 15 reports..."
                        value={searchTerm}
                            ...
                            <tr>
                                <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <p className="font-medium">No IFRS 15 reports found</p>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            IFRS 15 Report Details
                        </DialogTitle>
                        <DialogDescription>
                            Detailed view of IFRS 15 - Revenue from Contracts with Customers
                        </DialogDescription>
                    </DialogHeader>
                    {selectedReport && (
            ...
            <Dialog open={isGenerateModalOpen} onOpenChange={setIsGenerateModalOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Configure and generate a new IFRS 15 - Revenue Recognition report
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
                ...
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Export IFRS 15 Reports
                        </DialogTitle>
                        <DialogDescription>
                            Export IFRS 15 reports in your preferred format.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
