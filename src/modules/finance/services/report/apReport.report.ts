// src/services/finance/report/apReport.report.ts
...
export class APReport {
    // ============================================================
    // ✅ GENERATE PDF DIRECTLY
    // ============================================================
    static generatePDF(data: APReportData, companyName: string = 'RST ERP System'): jsPDF {
        const doc = new jsPDF({
        ...
        doc.text('ACCOUNTS PAYABLE REPORT', pageWidth / 2, yPos, { align: 'center' });
        yPos += 8;
        ...
        const cardHeight = 28;
        const cards = [
            { label: 'Total Payables', amount: data.totalPayables, color: '#2563eb' },
            { label: 'Total Paid', amount: data.totalPaid, color: '#16a34a' },
            { label: 'Overdue', amount: data.overduePayables, color: '#dc2626' },
            { label: 'Payment Rate', amount: data.paymentRate, color: '#7c3aed', isPercentage: true },
        ];
        ...
        const agingData = [
            ...
            ['Total', formatCurrency(data.totalPayables)],
        ];
        ...
        const statusData = [
            ['Pending', data.statusBreakdown.pending.toString()],
            ['Partially Paid', data.statusBreakdown.partiallyPaid.toString()],
            ['Approved', data.statusBreakdown.approved.toString()],
        ...
        if (data.topVendors && data.topVendors.length > 0) {
            ...
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(15, 23, 42);
            doc.text('Top Vendors by Payables', margin, yPos);
            yPos += 6;

            const topVendorData = data.topVendors.map(item => [
                item.name,
            ...
            autoTable(doc, {
                startY: yPos,
                head: [['Vendor', 'Invoices', 'Amount', 'Percentage']],
                body: topVendorData,
                margin: { left: margin, right: margin },
                styles: {
                    fontSize: 8,
        ...
        if (data.periodBreakdown && data.periodBreakdown.length > 1) {
            ...
            autoTable(doc, {
                startY: yPos,
                head: [['Period', 'Invoices', 'Total', 'Paid', 'Balance']],
                body: periodData,
                margin: { left: margin, right: margin },
                styles: {
                    fontSize: 8,
    ...
    // ============================================================
    static generateHTML(data: APReportData, companyName: string = 'RST ERP'): string {
        const periodDisplay = data.periodName || data.period || 'N/A';
    ...
    <div class="report-container">
        ...
        <div class="summary-grid">
            <div class="summary-card blue">
                ...
                <div class="value">${formatCurrency(data.totalPayables)}</div>
            </div>
            ...
            <div class="summary-card red">
                ...
                <div class="value">${formatCurrency(data.overduePayables)}</div>
            </div>
            <div class="summary-card purple">
                <div class="label">Payment Rate</div>
                <div class="value">${data.paymentRate.toFixed(1)}%</div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr style="font-weight:700;border-top:2px solid #1a1a2e;"><td>Total</td><td class="text-right">${formatCurrency(data.totalPayables)}</td></tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table>
                ...
                <tbody>
                    <tr><td>Pending</td><td class="text-right">${data.statusBreakdown.pending}</td></tr>
                    <tr><td>Partially Paid</td><td class="text-right">${data.statusBreakdown.partiallyPaid}</td></tr>
                    <tr><td>Approved</td><td class="text-right">${data.statusBreakdown.approved}</td></tr>
        ...
        <div class="section">
            <div class="section-title">Top Vendors by Payables</div>
            <table>
                <thead><tr><th>Vendor</th><th class="text-center">Invoices</th><th class="text-right">Amount</th><th class="text-right">%</th></tr></thead>
                <tbody>
        ...
        <div class="section">
            ...
            <table>
                <thead><tr><th>Period</th><th class="text-center">Invoices</th><th class="text-right">Total</th><th class="text-right">Paid</th><th class="text-right">Balance</th></tr></thead>
                <tbody>
