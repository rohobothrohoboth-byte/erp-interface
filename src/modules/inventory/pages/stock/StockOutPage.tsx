...
export default function StockOutPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [moves, prods, whs] = await Promise.all([
        stockApi.getMovements({ type: "Outbound" }),
      ...
      const message = err?.message || "Failed to load stock out movements";
      setError(message);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast.success("Stock out movement posted");
      setDialogOpen(false);
      await loadData();
    } catch (err: any) {
      showToast.error(err?.message || "Failed to post stock out movement");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      {loading ? (
        <div className="flex flex-col items-center justify-center py-16 text-slate-500">
          ...
          <p className="text-sm">Loading stock out movements...</p>
        </div>
      ) : error ? (
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={99} className="px-4 py-12 text-center text-slate-400">
                    ...
                    <p className="text-sm font-medium text-slate-500">No stock out movements</p>
                    <p className="mt-1 text-xs text-slate-400">
                      Post your first stock out movement using the button above.
                    </p>
                  </td>
                </tr>
              )}
