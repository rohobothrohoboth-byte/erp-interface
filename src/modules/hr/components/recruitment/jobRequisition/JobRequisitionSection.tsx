// src/components/hr/recruitment/jobRequisition/JobRequisitionSection.tsx
...
import JobRequisitionSearchFilter from '@/modules/hr/components/recruitment/jobRequisition/JobRequisitionSearchFilter';
import JobRequisitionTable from '@/modules/hr/components/recruitment/jobRequisition/JobRequisitionTable';
import AddJobRequisitionModal from '@/modules/hr/components/recruitment/jobRequisition/AddJobRequisitionModal';
import EditJobRequisitionModal from '@/modules/hr/components/recruitment/jobRequisition/EditJobRequisitionModal';
import DeleteJobRequisitionModal from '@/modules/hr/components/recruitment/jobRequisition/DeleteJobRequisitionModal';
import JobRequisitionReviewModal from '@/modules/hr/components/recruitment/jobRequisition/JobRequisitionReviewModal';
...
import { useCreateJobPosting, useCreateAllJobPosting } from '@/modules/hr/services/recruitment/jobPosting/jobPosting.queries';
import type { JobReqListDto, JobReqAddDto, JobReqModDto, UUID } from '@/modules/hr/types/recruit/jobRequisition';
...
const JobRequisitionSection: React.FC<JobRequisitionSectionProps> = ({ workforcePlanId, workforcePlanCode }) => {
    ...
    // ✅ Fetch requisitions for this workforce plan
    const {
        data: allItems = [],
        isLoading,
        error,
    ...
    const createMutation = useCreateJobRequisition({
        onSuccess: () => {
            showToast.success('Job requisition added successfully');
            setIsAddOpen(false);
            refetch();
        },
    ...
    const createAllPostingMutation = useCreateAllJobPosting({
        onSuccess: () => {
            showToast.success('All job postings created successfully');
    ...
    const deleteMutation = useDeleteJobRequisition({
        onSuccess: () => {
            showToast.success('Job requisition deleted successfully');
    ...
    return (
        <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-gray-50 space-y-6 min-h-screen p-6">
            ...
            <DeleteJobRequisitionModal
                isOpen={!!deletingItem}
                reqNumber={deletingItem?.reqNumber ?? ''}
            ...
            <AddJobPostingModal
                ...
                reqNumber={postingItem?.reqNumber}
