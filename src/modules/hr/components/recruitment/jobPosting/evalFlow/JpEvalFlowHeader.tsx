import React from 'react';
import { ArrowLeft, ClipboardCheck } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { Button } from '@/shared/components/ui/button';

interface JpEvalFlowHeaderProps {
  displayPostNumber: string;
}

const JpEvalFlowHeader: React.FC<JpEvalFlowHeaderProps> = ({ displayPostNumber }) => {
  const navigate = useNavigate();
  return (
    <div className="flex items-center gap-3">
      <Button variant="outline" onClick={() => navigate(-1)} className="flex items-center gap-2 px-3 py-2 cursor-pointer">
        <ArrowLeft className="w-4 h-4" />
        <span className="text-sm font-medium">Back</span>
      </Button>
      <div className="flex items-center gap-2">
        <ClipboardCheck className="w-6 h-6 text-green-600" />
        <div>
          <h1 className="text-2xl font-bold">
            <span className="bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
              Evaluation Flows
            </span>
          </h1>
          {displayPostNumber && <p className="text-sm text-gray-500">Job Posting: {displayPostNumber}</p>}
        </div>
      </div>
    </div>
  );
};

export default JpEvalFlowHeader;
