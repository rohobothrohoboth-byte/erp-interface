// src/services/hr/recruitment/jpEvalFlow/jpEvalFlow.queries.ts

import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { jpEvalFlowApi } from '@/modules/hr/services/recruitment/jpEvalFlow/jpEvalFlow.api';
import { jpEvalFlowKeys } from '@/modules/hr/services/recruitment/jpEvalFlow/jpEvalFlow.key';
...
export const useJpEvalFlows = (postId: string) => {
    return useQuery<JpEvalFlowListDto[], Error>({
        queryKey: jpEvalFlowKeys.byPost(postId),
        queryFn: () => jpEvalFlowApi.getByPost(postId),
        enabled: !!postId,
        staleTime: 0,
        refetchOnMount: 'always',
        // ✅ No need for retry logic since API now returns empty array on error
        retry: false,
    });
};
