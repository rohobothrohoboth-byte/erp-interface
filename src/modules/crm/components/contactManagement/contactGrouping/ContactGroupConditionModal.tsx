...
interface ContactGroupConditionModalProps {
  isOpen: boolean;
  onClose: () => void;
  groupId: string;
  groupName: string;
...
const mockConditionsStorage: Record<string, ContactGroupCondition[]> = {
  '1': [
    { id: '1', field: 'Stage', operator: 'Equals', value: 'Customer' },
    { id: '2', field: 'Company', operator: 'Contains', value: 'Tech' }
  ],
  '2': [
    { id: '1', field: 'Stage', operator: 'Equals', value: 'Prospect' }
  ]
};
...
const stageOptions = ['Lead', 'Prospect', 'Customer', 'Partner'];
const tagOptions = ['VIP', 'Enterprise', 'SMB', 'Startup', 'Government'];

export default function ContactGroupConditionModal({
  isOpen,
  onClose,
  groupId,
  groupName
}: ContactGroupConditionModalProps) {
  const [conditions, setConditions] = useState<ContactGroupCondition[]>([]);
  ...
  const renderValueInput = () => {
    ...
    if (['Relationship Score'].includes(formData.field)) {
      return (
        <Input
          name="value"
          type="number"
