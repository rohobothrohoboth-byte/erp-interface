...
import type { JobDescriptionListDto } from '../../../../../types/hr/jobDescription';

interface JobDescTableProps {
  items: JobDescriptionListDto[];
  onEdit: (item: JobDescriptionListDto) => void;
  onDelete: (item: JobDescriptionListDto) => void;
  onToggleActive: (item: JobDescriptionListDto) => void;
}

const PAGE_SIZE = 10;

const JobDescTable: React.FC<JobDescTableProps> = ({ items, onEdit, onDelete, onToggleActive }) => {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginated.length === 0 && (
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center">
                  ...
                  <p className="text-sm font-medium text-gray-900 mb-1">No Job Descriptions Found</p>
            ...
            {paginated.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-700">{item.department}</td>
                <td className="px-4 py-2 whitespace-nowrap">
      ...
      {items.length > 0 && (
        <div className="bg-white px-6 py-3 flex items-center justify-between border-t border-gray-200">
          <p className="text-sm text-gray-700">
            ...
            <span className="font-medium">{items.length}</span> entries
          </p>
