...
import { useProfileEmContact, profileKeys } from '../../services/profile/profile.queries';
import { useProfileStore } from '../../stores/profile/profile.store';
import type { EmergencyContactForm } from '../../stores/profile/profile.store';
...
const ADDR_TYPE_OPTS   = Object.entries(AddressType).map(([key, label]) => ({ key, label }));

const relationKey    = (label: string) => Object.entries(Relation).find(([, v]) => v === label)?.[0] ?? label;
const genderKey      = (label: string) => Object.entries(Gender).find(([, v]) => v === label)?.[0] ?? label;
const addrTypeKey    = (label: string) => Object.entries(AddressType).find(([, v]) => v === label)?.[0] ?? label;

const EMPTY_FORM: EmergencyContactForm = {
  firstName: '', middleName: '', lastName: '',
  nationality: '', gender: '', relation: '', telephone: '',
  country: '', region: '', subcity: '', zone: '',
  woreda: '', kebele: '', houseNo: '', poBox: '',
  addressType: '', fax: '', email: '', website: '',
};
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            <EditableField label="First Name"  value={form.firstName}   isEditing={isEditing} onChange={(v) => setField('firstName', v)} />
            <EditableField label="Middle Name" value={form.middleName}  isEditing={isEditing} onChange={(v) => setField('middleName', v)} />
            <EditableField label="Last Name"   value={form.lastName}    isEditing={isEditing} onChange={(v) => setField('lastName', v)} />
            <EditableField label="Nationality" value={form.nationality} isEditing={isEditing} onChange={(v) => setField('nationality', v)} />
            <EditableField label="Telephone"   value={form.telephone}   isEditing={isEditing} onChange={(v) => setField('telephone', v)} />
            <EditableField label="Email"       value={form.email}       isEditing={isEditing} onChange={(v) => setField('email', v)} />
          ...
          <div className="flex-1">
            ...
            {isEditing && (
              <div className="flex flex-col gap-1">
                ...
                <Select value={form.addressType} onValueChange={(v) => setField('addressType', v)}>
                  <SelectTrigger className="w-full text-sm border-gray-200 focus:border-green-400">
            ...
            <EditableField label="Country"   value={form.country}  isEditing={isEditing} onChange={(v) => setField('country', v)} />
            <EditableField label="Region"    value={form.region}   isEditing={isEditing} onChange={(v) => setField('region', v)} />
            <EditableField label="Subcity"   value={form.subcity}  isEditing={isEditing} onChange={(v) => setField('subcity', v)} />
            <EditableField label="Zone"      value={form.zone}     isEditing={isEditing} onChange={(v) => setField('zone', v)} />
            <EditableField label="Woreda"    value={form.woreda}   isEditing={isEditing} onChange={(v) => setField('woreda', v)} />
            <EditableField label="Kebele"    value={form.kebele}   isEditing={isEditing} onChange={(v) => setField('kebele', v)} />
            <EditableField label="House No." value={form.houseNo}  isEditing={isEditing} onChange={(v) => setField('houseNo', v)} />
            <EditableField label="P.O. Box"  value={form.poBox}    isEditing={isEditing} onChange={(v) => setField('poBox', v)} />
            <EditableField label="Fax"       value={form.fax}      isEditing={isEditing} onChange={(v) => setField('fax', v)} />
            {isEditing ? (
              <EditableField label="Website" value={form.website}  isEditing onChange={(v) => setField('website', v)} />
            ) : (
