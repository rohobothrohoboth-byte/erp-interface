...
import { BadgePlus, Search, X, SlidersHorizontal, ChevronDown } from 'lucide-react';
...
import { EmpState, Gender } from '../../../types/hr/enum';
import { useDepartmentNames, useBranchComp } from '../../../services/List/hrmmNames/hrmmNames.queries';
import { useRoles } from '../../../services/List/auth/authList.queries';
...
const EmployeeSearch: React.FC<EmployeeSearchProps> = ({
  ...
  const handleAddEmployee = onAddEmployee || (() => navigate('/core/add-employee'));
  const clearSearch = () => setSearchTerm('');

  const handleFilterChange = (key: keyof AdminEmployeeFilters, value: string) => {
    setFilters({ ...filters, [key]: value === '__all__' ? '' : value });
  };

  const clearAllFilters = () => {
    setFilters({ department: '', branch: '', empState: '', role: '', gender: '' });
    setSearchTerm('');
  };
