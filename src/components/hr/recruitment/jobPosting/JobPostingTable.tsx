      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-white">
            <tr>
              {['Post Number', 'Req Number', 'Type', 'Status', 'Deadline', 'Qty', 'Actions'].map((h) => (
              ...
              <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs font-medium text-gray-900">{item.postNumber}</span>
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 font-mono">{item.reqNumber}</td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${typeColors[item.postTypeStr] ?? 'bg-gray-100 text-gray-600'}`}>
                    {item.postTypeStr}
                  </span>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
