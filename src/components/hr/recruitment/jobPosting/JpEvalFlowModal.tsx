...
interface Props { isOpen: boolean; posting: JobPostingListDto | null; onClose: () => void; }
...
const FlowStepsPreview: React.FC<{ flowId: string }> = ({ flowId }) => {
  ...
  React.useEffect(() => {
    setLoaded(false);
    evaluationStepApi.getAllByFlow(flowId).then(s => { setSteps(s); setLoaded(true); }).catch(() => setLoaded(true));
  }, [flowId]);
    ...
    <div className="space-y-1.5 pt-1">
      {steps.map((s, i) => (
        <div key={s.id} className="flex items-center gap-2 text-xs">
          <span className="w-4 h-4 rounded-full bg-green-100 text-green-700 flex items-center justify-center font-bold shrink-0 text-[10px]">{i + 1}</span>
          ...
          {s.isFinalStr === 'Yes' && <span className="bg-green-100 text-green-700 px-1.5 py-0.5 rounded text-[10px] font-semibold shrink-0">Final</span>}
        </div>
      ))}
    </div>
  );
...
const JpEvalFlowModal: React.FC<Props> = ({ isOpen, posting, onClose }) => {
  ...
  const [steps, setSteps] = useState<StepRow[]>([{ stepName: '', stepOrder: 1, isFinal: false, evalTypeId: '' }]);
  const [isCreating, setIsCreating] = useState(false);

  // Right panel
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const { data: items = [], isLoading, refetch: refetchItems } = useJpEvalFlows(postId);
  ...
  React.useEffect(() => {
    if (!isOpen) {
      setLeftMode('assign');
      setEditingItem(null);
      setFlowId(''); setEffectiveFrom(''); setPreviewFlowId(null);
      setNewFlowName(''); setNewFlowGlobal(false);
      setSteps([{ stepName: '', stepOrder: 1, isFinal: false, evalTypeId: '' }]);
      setExpandedId(null);
    }
  }, [isOpen]);

  const createJpMutation = useCreateJpEvalFlow({
    onSuccess: () => { showToast.success('Evaluation flow assigned'); resetAssign(); refetchItems(); },
    onError: (e) => showToast.error(e.message),
  });
  const updateJpMutation = useUpdateJpEvalFlow(postId, {
    onSuccess: () => { showToast.success('Evaluation flow updated'); resetAssign(); refetchItems(); },
    onError: (e) => showToast.error(e.message),
  });
  const deleteJpMutation = useDeleteJpEvalFlow(postId, {
    onSuccess: () => { showToast.success('Evaluation flow removed'); refetchItems(); },
    onError: (e) => showToast.error(e.message),
  });
  const createFlowMutation = useCreateEvaluationFlow();

  const resetAssign = () => { setEditingItem(null); setFlowId(''); setEffectiveFrom(''); setPreviewFlowId(null); };
  const resetCreate = () => { setNewFlowName(''); setNewFlowGlobal(false); setSteps([{ stepName: '', stepOrder: 1, isFinal: false, evalTypeId: '' }]); };

  const startEdit = (item: JpEvalFlowListDto) => {
    setEditingItem(item);
    // Pre-select the flow by matching name in the evalFlows list
    const matched = evalFlows.find(f => f.name === item.evalFlowName);
    setFlowId(matched?.id ?? '');
    setPreviewFlowId(matched?.id ?? null);
    // Pre-fill effective from — parse the display string back to YYYY-MM-DD
    try {
  ...
  const handleAssignSubmit = () => {
    ...
    if (editingItem) {
      updateJpMutation.mutate({ id: editingItem.id, evaluationFlowId: flowId, effectiveFrom: iso, rowVersion: editingItem.rowVersion });
    } else {
      createJpMutation.mutate({ evaluationFlowId: flowId, jobPostingId: postId, effectiveFrom: iso });
    }
  };
  ...
  const handleCreateFlow = async () => {
    ...
    try {
      ...
      resetCreate();
      setFlowId(newFlow.id); setPreviewFlowId(newFlow.id);
      setLeftMode('assign');
    } catch (e: any) {
  ...
  const addStep = () => setSteps(s => [...s, { stepName: '', stepOrder: s.length + 1, isFinal: false, evalTypeId: '' }]);
  const removeStep = (i: number) => setSteps(s => s.filter((_, idx) => idx !== i).map((r, idx) => ({ ...r, stepOrder: idx + 1 })));
  const updateStep = (i: number, field: keyof StepRow, val: any) => setSteps(s => s.map((r, idx) => idx === i ? { ...r, [field]: val } : r));
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        className="bg-white rounded-xl shadow-2xl w-full max-w-4xl h-[82vh] flex flex-col">
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-200 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-400 truncate">{posting.postNumber} · {posting.reqNumber}</p>
          </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-1/2 border-r border-gray-200 flex flex-col overflow-hidden">
            ...
            <div className="flex border-b border-gray-200 shrink-0">
              <button type="button"
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium transition-colors cursor-pointer ${leftMode === 'assign' ? 'text-green-700 border-b-2 border-green-600 bg-green-50/50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                <Plus size={14} /> {editingItem ? 'Edit' : 'Assign'}
              </button>
              <button type="button"
                onClick={() => { setLeftMode('create'); resetCreate(); }}
                className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium transition-colors cursor-pointer ${leftMode === 'create' ? 'text-green-700 border-b-2 border-green-600 bg-green-50/50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                ...
                <>
                  {editingItem && (
                    <div className="flex items-center justify-between bg-green-50 rounded-lg px-3 py-2">
                      <p className="text-xs font-medium text-green-700">Editing: {editingItem.evalFlowName}</p>
                      <button type="button" onClick={resetAssign} className="text-green-500 hover:text-green-700 cursor-pointer"><X size={13} /></button>
                    </div>
                  )}
                  ...
                  <div className="space-y-1.5">
                      ...
                      {evalFlows.length === 0 ? (
                        ...
                        <div key={f.id}>
                          <button type="button"
                            ...
                            className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors cursor-pointer ${flowId === f.id ? 'bg-green-50' : 'hover:bg-gray-50'}`}>
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${flowId === f.id ? 'border-green-600 bg-green-600' : 'border-gray-300'}`}>
                              {flowId === f.id && <Check size={9} className="text-white" />}
                            </div>
                            <span className={`text-sm flex-1 truncate ${flowId === f.id ? 'font-semibold text-green-800' : 'text-gray-700'}`}>{f.name}</span>
                            <button type="button" onClick={e => { e.stopPropagation(); setPreviewFlowId(previewFlowId === f.id ? null : f.id); }}
                              className="text-gray-400 hover:text-green-600 cursor-pointer shrink-0">
                              <Eye size={13} />
                            </button>
                          </button>
                          <AnimatePresence>
                            {previewFlowId === f.id && (
                              <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }}
                                className="overflow-hidden bg-green-50/60 border-t border-green-100 px-3 pb-2">
                    ...
                    <button type="button" onClick={() => { setLeftMode('create'); resetCreate(); }}
                      className="flex items-center gap-1 text-sm text-green-600 hover:text-green-700 hover:underline cursor-pointer mt-1">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input type="date" value={effectiveFrom} onChange={e => setEffectiveFrom(e.target.value)} disabled={isAssignSubmitting} className="h-10 text-sm" />
                  </div>

                  <Button type="button" onClick={handleAssignSubmit}
                    disabled={!flowId || !effectiveFrom || isAssignSubmitting}
                    className="w-full bg-green-600 hover:bg-green-700 text-white cursor-pointer rounded-lg">
                ...
                <>
                  <div className="space-y-1.5">
                    ...
                    <Input value={newFlowName} onChange={e => setNewFlowName(e.target.value)}
                      placeholder="e.g. Technical Interview Flow" disabled={isCreating} className="h-10 text-sm" />
                  </div>

                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={newFlowGlobal} onChange={e => setNewFlowGlobal(e.target.checked)}
                      disabled={isCreating} className="w-4 h-4 rounded accent-green-600 cursor-pointer" />
                        ...
                        <div key={i} className="rounded-lg border border-gray-200 bg-gray-50/50 p-3 space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="flex items-center gap-1.5 text-sm font-semibold text-gray-500">
                              <span className="w-5 h-5 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-xs font-bold">{s.stepOrder}</span>
                              Step {s.stepOrder}
                            </span>
                            {steps.length > 1 && (
                              <button type="button" onClick={() => removeStep(i)} disabled={isCreating}
                                className="text-gray-400 hover:text-red-500 cursor-pointer"><X size={13} /></button>
                            )}
                          </div>
                          <div className="space-y-1.5">
                            <Input value={s.stepName} onChange={e => updateStep(i, 'stepName', e.target.value)}
                              placeholder="Step name" disabled={isCreating} className="h-9 text-sm" />
                            <Select value={s.evalTypeId} onValueChange={v => updateStep(i, 'evalTypeId', v)} disabled={isCreating}>
                              <SelectTrigger className="h-9 text-sm w-full"><SelectValue placeholder="Evaluation type" /></SelectTrigger>
                              <SelectContent>
                                {evalTypes.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
                              </SelectContent>
                            </Select>
                          </div>
                          <label className="flex items-center gap-2 cursor-pointer">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200 shrink-0">
              ...
              <p className="text-xs text-gray-400">{items.length} flow{items.length !== 1 ? 's' : ''} assigned</p>
            </div>
              ...
              {isLoading ? (
                ...
                <div key={item.id} className="rounded-xl border border-gray-200 overflow-hidden">
                  <div className="flex items-center gap-2.5 px-3 py-3 bg-gray-50/80">
                    ...
                    <button type="button" onClick={() => deleteJpMutation.mutate(item.id)} disabled={deleteJpMutation.isPending}
                        ...
                        <div className="px-3 py-3 space-y-2">
                          {item.steps.map((s, i) => (
                            <div key={i} className="flex items-center gap-2.5 text-xs">
                              <span className="w-5 h-5 rounded-full bg-green-100 text-green-700 flex items-center justify-center font-bold shrink-0 text-[10px]">{i + 1}</span>
                              <span className="font-medium text-gray-700 flex-1 truncate">{s.stepName}</span>
                              <span className="text-gray-400 shrink-0">{s.evalType}</span>
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-gray-200 bg-gray-50 flex justify-end rounded-b-xl shrink-0">
          <Button variant="outline" onClick={onClose} className="cursor-pointer rounded-lg">Close</Button>
        </div>
      </motion.div>
    </div>
  );
