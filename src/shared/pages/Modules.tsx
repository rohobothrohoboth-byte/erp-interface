// src/shared/pages/Modules.tsx - Navy & Gold Theme
...
const GRADIENT_MAP: Record<string, string> = {
  "mod.core": "from-[#c9a84c] to-[#d4af37]",
  "mod.hrm": "from-[#c9a84c] to-[#d4af37]",
  "mod.fnm": "from-[#c9a84c] to-[#d4af37]",
  "mod.inv": "from-[#c9a84c] to-[#d4af37]",
  "mod.crm": "from-[#c9a84c] to-[#d4af37]",
  "mod.pro": "from-[#c9a84c] to-[#d4af37]",
  "mod.pld": "from-[#c9a84c] to-[#d4af37]",
  "mod.prm": "from-[#c9a84c] to-[#d4af37]",
  "mod.flm": "from-[#c9a84c] to-[#d4af37]",
  "mod.rpt": "from-[#c9a84c] to-[#d4af37]",
  "default": "from-[#c9a84c] to-[#d4af37]",
};
...
function Modules() {
  ...
  // ✅ Set active module when permissions are loaded
  useEffect(() => {
  ...
  if (isLoading || loadingModules || loadingPermissions) {
    return (
        <div className="min-h-screen flex items-center justify-center bg-navy">
          <div className="animate-spin rounded-full h-12 w-12 border-2 border-gold border-t-transparent" />
        </div>
    );
  }
  ...
  return (
      <div className="min-h-screen bg-navy overflow-y-auto">
        ...
        {/* ✅ Header - Sticky with Logo & Company Name */}
        <header className="sticky top-0 z-50 bg-navy/95 backdrop-blur-xl border-b border-gold/20 shadow-lg shadow-gold/5">
          <div className="container mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between gap-2 sm:gap-4">
            {/* Logo - With Company Name */}
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl border-2 border-gold shadow-gold/20 overflow-hidden flex-shrink-0 bg-navy">
                {companyLoading ? (
                    <div className="w-full h-full animate-pulse bg-gold/10" />
                ) : company?.logoUrl ? (
                    <img
                        src={company.logoUrl}
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            ...
                            fallback.className = 'w-full h-full flex items-center justify-center text-base font-bold text-gold';
                            fallback.textContent = company?.name?.charAt(0)?.toUpperCase() || 'R';
                            parent.appendChild(fallback);
                          }
                        }}
                    />
                ) : (
                    <span className="w-full h-full flex items-center justify-center text-base font-bold text-gold">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-[10px] sm:text-xs text-gold-dim">
                  {company?.motto || t.enterpriseResourcePlanning}
                </p>
              </div>
            </div>

            {/* Center - Organization Info - Hidden on mobile */}
            <div className="hidden lg:flex items-center gap-4 text-gold-dim flex-1 justify-center min-w-0">
              {hasOrgInfo ? (
                  <div className="flex items-center gap-3 px-3 py-1.5 bg-navy-light rounded-lg border border-gold/20">
                    {branchName && (
                        <>
                          <div className="flex items-center gap-1.5 text-xs">
                            <Building2 size={12} className="text-gold" />
                            <span className="font-medium text-gold truncate max-w-[120px]">
                              {branchName}
                            </span>
                          </div>
                  ...
                  <div className="text-xs text-gold-faint">No Organization Assigned</div>
              )}

              {/* Date/Time */}
              <div className="flex items-center gap-2 text-gold-dim flex-shrink-0">
                <Activity size={14} />
            ...
            <div className="flex lg:hidden items-center gap-1 sm:gap-2 text-[10px] sm:text-xs text-gold-dim truncate max-w-[100px] sm:max-w-[150px] flex-1 justify-center">
              <Building2 size={10} className="text-gold flex-shrink-0" />
              <span className="truncate">{shortOrgDisplay}</span>
            </div>

            {/* Actions */}
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
              <button onClick={toggleDarkMode} className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl text-gold-dim hover:text-gold hover:bg-gold-subtle transition-colors">
                {isDarkMode ? <Sun size={16} className="sm:w-5 sm:h-5" /> : <Moon size={16} className="sm:w-5 sm:h-5" />}
              </button>

              <Popover open={helpOpen} onOpenChange={setHelpOpen}>
                <PopoverTrigger asChild>
                  <button className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl text-gold-dim hover:text-gold hover:bg-gold-subtle transition-colors">
                    <HelpCircle className="w-4 h-4 sm:w-5 sm:h-5" />
                  </button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-56 sm:w-64 p-0 rounded-2xl overflow-hidden shadow-xl bg-navy-light border border-gold/20">
                  <div className="p-3 border-b border-gold/10">
                    <p className="text-sm font-semibold text-gold">{t.helpAndSupport}</p>
                  </div>
                  <div className="p-2">
                    {[
                      ...
                      {icon: Zap, label: t.contactSupport, onClick: () => window.open('mailto:support@rst.com')}
                        ...
                        <button
                            ...
                            className="w-full text-left px-3 py-2 text-sm text-gold-dim hover:bg-gold-subtle rounded-lg flex items-center gap-2 transition-colors"
                        >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="focus:outline-none">
                    <Avatar className="w-8 h-8 sm:w-10 sm:h-10 ring-2 ring-gold/30 shadow-lg cursor-pointer">
                      <AvatarImage src="https://github.com/shadcn.png" />
                      <AvatarFallback className="bg-gold-gradient text-navy font-bold text-xs sm:text-sm">
                        {userName?.charAt(0)?.toUpperCase() || 'U'}
                      </AvatarFallback>
                    </Avatar>
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56 rounded-xl shadow-xl bg-navy-light border border-gold/20">
                  <DropdownMenuLabel>
                    <div className="text-sm font-semibold text-gold">{userName || t.employee}</div>
                    <div className="text-xs text-gold-dim capitalize">{role || t.employee}</div>
                    ...
                    <div className="mt-2 pt-2 border-t border-gold/10 space-y-1">
                      ...
                      {!branchName && !departmentName && !positionName && (
                          <div className="text-xs text-gold-faint italic">No organization assigned</div>
                      )}
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator className="bg-gold/10" />
                  <DropdownMenuItem onSelect={() => navigate("/profile")} className="text-gold-dim hover:text-gold hover:bg-gold-subtle">
                    <User className="w-4 h-4 mr-2" /> {t.profile}
                  </DropdownMenuItem>
                  {isPrivilegedRole && (
                      <DropdownMenuItem onSelect={() => navigate("/settings")} className="text-gold-dim hover:text-gold hover:bg-gold-subtle">
                        <Settings className="w-4 h-4 mr-2" /> {t.settings}
                      </DropdownMenuItem>
                  )}
                  <DropdownMenuItem onSelect={() => navigate("/modules")} className="text-gold-dim hover:text-gold hover:bg-gold-subtle">
                    <LayoutDashboard className="w-4 h-4 mr-2" /> {t.dashboard}
                  </DropdownMenuItem>
                  {isSuperAdmin && (
                      <DropdownMenuItem onSelect={() => navigate("/admin")} className="text-gold-dim hover:text-gold hover:bg-gold-subtle">
                        <Shield className="w-4 h-4 mr-2" /> {t.adminPanel}
                      </DropdownMenuItem>
                  )}
                  <DropdownMenuSeparator className="bg-gold/10" />
                  <DropdownMenuItem onSelect={handleLogout} className="text-red-400 hover:text-red-300 hover:bg-red-500/10">
        ...
        <main className="container mx-auto px-3 sm:px-4 py-3 sm:py-4 md:py-6 lg:py-8">
          {/* Welcome Section - Navy & Gold */}
          <div className="mb-4 sm:mb-5 md:mb-6 lg:mb-8">
            <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
              <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-gold flex-shrink-0" />
              <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gold truncate">
                {t.welcomeBack}, {userName || t.employee}
              </h2>
            </div>
            <p className="text-[10px] sm:text-xs md:text-sm lg:text-base text-gold-dim flex flex-wrap items-center gap-1">
              ...
              {hasOrgInfo && (
                  <>
                    ...
                    <span className="flex items-center gap-1 text-[10px] sm:text-xs md:text-sm">
                      ...
                      <span className="truncate max-w-[60px] xs:max-w-[80px] sm:max-w-[120px] md:max-w-[200px] text-gold/80">{branchName}</span>
                      {departmentName && (
                          <>
                            <span className="text-gold/30 hidden xs:inline">|</span>
                            <Users size={12} className="text-gold/80 flex-shrink-0 hidden xs:inline" />
                            <span className="truncate max-w-[60px] xs:max-w-[80px] sm:max-w-[120px] md:max-w-[150px] hidden xs:inline text-gold/80">{departmentName}</span>
                          </>
                      )}
                    </span>
                  </>
          ...
          <div className="flex flex-col lg:flex-row lg:gap-4 xl:gap-6">
            ...
            <div className="flex-1 min-w-0">
              {sortedModules.length === 0 ? (
                  <div className="text-center py-8 sm:py-10 md:py-12 lg:py-16 bg-navy-light/50 rounded-xl sm:rounded-2xl backdrop-blur-sm border border-gold/10">
                    <Shield className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 text-gold/40 mx-auto mb-2 sm:mb-3 md:mb-4" />
                    <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gold/80">{t.noModulesAvailable}</h3>
                    <p className="text-xs sm:text-sm text-gold-faint">{t.contactAdmin}</p>
                  </div>
              ) : (
                  <div className={`grid gap-2 sm:gap-3 md:gap-4 lg:gap-5 xl:gap-6 ${
                    ...
                    {sortedModules.map((module, index) => {
                      ...
                      return (
                          ...
                          >
                            <div className="relative p-2.5 sm:p-3 md:p-4 lg:p-5 xl:p-6 rounded-lg sm:rounded-xl md:rounded-2xl bg-navy-light border border-gold/20 shadow-lg shadow-gold/5 hover:shadow-gold/20 transition-all duration-300 overflow-hidden h-full">
                              ...
                              <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 rounded-lg sm:rounded-xl bg-gradient-to-br ${module.gradient} flex items-center justify-center mb-1.5 sm:mb-2 md:mb-3 lg:mb-4 shadow-lg group-hover:scale-110 transition-transform shadow-gold/20`}>
                                <IconComponent className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-7 lg:h-7 text-navy" />
                              </div>
                              <h3 className="text-xs sm:text-sm md:text-base lg:text-lg font-bold text-gold mb-0.5 sm:mb-1 truncate">
                                {module.name}
                              </h3>
                              <p className="text-[10px] sm:text-xs md:text-sm text-gold-dim mb-1 sm:mb-1.5 md:mb-2 lg:mb-3 line-clamp-2 hidden sm:block">
                                {getModuleDescription(module.descriptionKey)}
                              </p>
                              <div className="flex items-center justify-between">
                                <span className="text-[8px] sm:text-[10px] md:text-xs text-gold-faint">{t.clickToAccess}</span>
                                <ChevronRight className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 text-gold-faint group-hover:text-gold group-hover:translate-x-1 transition-all" />
                              </div>
                            </div>
                          </motion.div>
                      );
            ...
            <div className="mt-3 sm:mt-4 md:mt-6 lg:mt-0 lg:w-64 xl:w-72 2xl:w-80 lg:shrink-0">
              <div className="bg-navy-light rounded-lg sm:rounded-xl md:rounded-2xl border border-gold/20 overflow-hidden shadow-lg shadow-gold/5">
        ...
        {/* Footer */}
        <footer className="mt-4 sm:mt-6 md:mt-8 lg:mt-12 py-2 sm:py-3 md:py-4 lg:py-6 border-t border-gold/10">
          <div className="container mx-auto px-3 sm:px-4 text-center">
            <p className="text-[8px] sm:text-[10px] md:text-xs text-gold-faint">
              © {new Date().getFullYear()} {company?.name || 'RST ERP'}. {t.allRightsReserved} • {t.version} 2.0.0
            </p>
          </div>
        </footer>
