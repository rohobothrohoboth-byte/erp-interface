// src/pages/finance/ifrs/IFRS8.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { motion } from 'framer-motion';
import {
    Globe, Search, RefreshCw, Eye, Download, Printer,
    ...
    Building2, Users, Map, Compass, Layers,
    Split, GitBranch, Circle, Square, Hexagon
} from 'lucide-react';
...
const IFRS8: React.FC = () => {
    ...
    const {
        ...
        handleExport,
        handleRefresh,
    } = useReportExport('ifrs8');

    const ITEMS_PER_PAGE = 10;

    // Mock segment data - This would come from API in production
    const segmentData: SegmentData[] = [
        { name: 'North America', revenue: 2800000, assets: 8500000, profit: 420000, growth: 12.5 },
        { name: 'Europe', revenue: 1900000, assets: 5600000, profit: 280000, growth: 8.3 },
        { name: 'Asia Pacific', revenue: 1200000, assets: 3200000, profit: 120000, growth: 15.7 },
        { name: 'Other Regions', revenue: 600000, assets: 1500000, profit: -30000, growth: -2.1 },
    ];
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            console.error('Error fetching IFRS 8 reports:', error);
            showToast.error('Failed to load IFRS 8 reports');
        } finally {
            setLoading(false);
    ...
    const handleGenerate = async () => {
        try {
            ...
            showToast.success('IFRS 8 report generated successfully');
            await fetchData();
        ...
        >
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">IFRS 8 - Operating Segments</h1>
                        <p className="text-sm text-gray-500">Segment reporting - revenue, assets, and profit by operating segment</p>
                    </div>
                </div>
                ...
                <Card className="bg-gradient-to-r from-indigo-50 to-indigo-100 border-indigo-200">
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-indigo-700 font-medium">Total Reports</p>
                                <p className="text-2xl font-bold text-indigo-900">{stats.totalReports}</p>
                                <p className="text-xs text-indigo-600 mt-1">{stats.generatedReports} generated</p>
                            </div>
            ...
            {/* IFRS 8 Specific Cards - Segment Performance */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="border-l-4 border-l-blue-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Segment Revenue</h4>
                        <div className="space-y-3">
                            ...
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-bold text-blue-600">{formatCurrency(segmentData.reduce((sum, s) => sum + s.revenue, 0))}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="border-l-4 border-l-green-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Segment Assets</h4>
                        <div className="space-y-3">
                            {segmentData.map((segment, idx) => (
                                <div key={idx} className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">{segment.name}</span>
                                    <span className="font-semibold text-gray-900">{formatCurrency(segment.assets)}</span>
                                </div>
                            ))}
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total Assets</span>
                                    <span className="font-bold text-green-600">{formatCurrency(segmentData.reduce((sum, s) => sum + s.assets, 0))}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="border-l-4 border-l-purple-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Segment Profit</h4>
                        <div className="space-y-3">
                            {segmentData.map((segment, idx) => (
                                <div key={idx} className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">{segment.name}</span>
                                    <span className={`font-semibold ${segment.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-bold text-purple-600">{formatCurrency(segmentData.reduce((sum, s) => sum + s.profit, 0))}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <h4 className="text-sm font-semibold text-gray-700 mb-3">Operating Segment Summary</h4>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center p-3 bg-blue-50 rounded-lg">
                        <p className="text-xs text-gray-500">Total Segments</p>
                        <p className="text-2xl font-bold text-blue-600">{segmentData.length}</p>
                    </div>
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                        <p className="text-xs text-gray-500">Total Revenue</p>
                        <p className="text-2xl font-bold text-green-600">{formatCurrency(segmentData.reduce((sum, s) => sum + s.revenue, 0))}</p>
                    </div>
                    <div className="text-center p-3 bg-purple-50 rounded-lg">
                        <p className="text-xs text-gray-500">Total Assets</p>
                        <p className="text-2xl font-bold text-purple-600">{formatCurrency(segmentData.reduce((sum, s) => sum + s.assets, 0))}</p>
                    </div>
                    <div className="text-center p-3 bg-indigo-50 rounded-lg">
                        <p className="text-xs text-gray-500">Total Profit</p>
                        <p className="text-2xl font-bold text-indigo-600">{formatCurrency(segmentData.reduce((sum, s) => sum + s.profit, 0))}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-wrap gap-4">
                <div className="flex-1 min-w-[200px] relative">
                    ...
                    <Input
                        placeholder="Search IFRS 8 reports..."
                        value={searchTerm}
                            ...
                            <tr>
                                <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <p className="font-medium">No IFRS 8 reports found</p>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            IFRS 8 Report Details
                        </DialogTitle>
                        <DialogDescription>
                            Detailed view of IFRS 8 - Operating Segments report
                        </DialogDescription>
                    </DialogHeader>
                    {selectedReport && (
                ...
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Generate IFRS 8 Report
                        </DialogTitle>
                        <DialogDescription>
                            Configure and generate a new IFRS 8 - Operating Segments report
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
                ...
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Export IFRS 8 Reports
                        </DialogTitle>
                        <DialogDescription>
                            Export IFRS 8 reports in your preferred format.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
