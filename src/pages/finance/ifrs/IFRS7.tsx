// src/pages/finance/ifrs/IFRS7.tsx
...
import {
    ...
    X, Calendar, Target, Zap, Award,
    BookOpen, BarChart3, LineChart, AlertTriangle,
    ListChecks, FileCheck, Clipboard, Layers,
...
const IFRS7: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            console.error('Error fetching IFRS 7 reports:', error);
            showToast.error('Failed to load IFRS 7 reports');
        } finally {
            setLoading(false);
    ...
    const handleGenerate = async () => {
        try {
            ...
            showToast.success('IFRS 7 report generated successfully');
            await fetchData();
        ...
        >
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">IFRS 7 - Disclosures</h1>
                        <p className="text-sm text-gray-500">Financial instruments disclosures - fair value, risk exposures, and sensitivity analysis</p>
                    </div>
                </div>
            ...
            {/* IFRS 7 Specific Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="border-l-4 border-l-blue-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Fair Value Hierarchy</h4>
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Level 1 - Quoted Prices</span>
                                <span className="font-semibold text-gray-900">{formatCurrency(8500000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Level 2 - Observable Inputs</span>
                                <span className="font-semibold text-gray-900">{formatCurrency(12000000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Level 3 - Unobservable Inputs</span>
                                <span className="font-semibold text-gray-900">{formatCurrency(3200000)}</span>
                            </div>
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total Fair Value</span>
                                    <span className="font-bold text-blue-600">{formatCurrency(23700000)}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="border-l-4 border-l-green-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Risk Exposures</h4>
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Market Risk</span>
                                <span className="font-semibold text-yellow-600">{formatCurrency(2800000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Credit Risk</span>
                                <span className="font-semibold text-red-600">{formatCurrency(4500000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Liquidity Risk</span>
                                <span className="font-semibold text-blue-600">{formatCurrency(1800000)}</span>
                            </div>
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total Risk Exposure</span>
                                    <span className="font-bold text-gray-900">{formatCurrency(9100000)}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="border-l-4 border-l-purple-500">
                    <CardContent className="p-4">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Interest Rate ±1%</span>
                                <span className="font-semibold text-blue-600">±{formatCurrency(320000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">FX Rate ±5%</span>
                                <span className="font-semibold text-green-600">±{formatCurrency(580000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">Equity Price ±10%</span>
                                <span className="font-semibold text-orange-600">±{formatCurrency(420000)}</span>
                            </div>
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Maximum Impact</span>
                                    <span className="font-bold text-red-600">±{formatCurrency(1320000)}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-wrap gap-4">
                <div className="flex-1 min-w-[200px] relative">
                    ...
                    <Input
                        placeholder="Search IFRS 7 reports..."
                        value={searchTerm}
                            ...
                            <tr>
                                <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <p className="font-medium">No IFRS 7 reports found</p>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            IFRS 7 Report Details
                        </DialogTitle>
                        <DialogDescription>
                            Detailed view of IFRS 7 - Financial Instruments Disclosures
                        </DialogDescription>
                    </DialogHeader>
                    {selectedReport && (
                ...
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Generate IFRS 7 Report
                        </DialogTitle>
                        <DialogDescription>
                            Configure and generate a new IFRS 7 - Disclosures report
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
                ...
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Export IFRS 7 Reports
                        </DialogTitle>
                        <DialogDescription>
                            Export IFRS 7 reports in your preferred format.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
