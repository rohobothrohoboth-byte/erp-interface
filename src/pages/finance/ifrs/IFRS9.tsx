...
const IFRS9: React.FC = () => {
    ...
    const {
        ...
        handleExport,
        handleRefresh,
    } = useReportExport('ifrs9');

    const ITEMS_PER_PAGE = 10;

    // Mock data - These would come from API in production
    const classificationData: ClassificationData[] = [
        { category: 'Amortized Cost', amount: 45200000, percentage: 50.8 },
        { category: 'FVOCI', amount: 28700000, percentage: 32.3 },
        { category: 'FVTPL', amount: 15300000, percentage: 16.9 },
    ];

    const eclData: ECLData[] = [
        { stage: 'Stage 1 (12-month)', amount: 2100000, description: 'Performing assets' },
        { stage: 'Stage 2 (Lifetime)', amount: 5800000, description: 'Significant increase in credit risk' },
        { stage: 'Stage 3 (Credit-impaired)', amount: 3400000, description: 'Non-performing assets' },
    ];

    const hedgeData: HedgeData[] = [
        { type: 'Fair Value Hedges', amount: 12500000, description: 'Hedge of exposure to changes in fair value' },
        { type: 'Cash Flow Hedges', amount: 8900000, description: 'Hedge of exposure to variability in cash flows' },
        { type: 'Net Investment Hedges', amount: 6300000, description: 'Hedge of net investment in foreign operations' },
    ];
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            console.error('Error fetching IFRS 9 reports:', error);
            showToast.error('Failed to load IFRS 9 reports');
        } finally {
            setLoading(false);
    ...
    const handleGenerate = async () => {
        try {
            ...
            showToast.success('IFRS 9 report generated successfully');
            await fetchData();
        ...
        >
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">IFRS 9 - Financial Instruments</h1>
                        <p className="text-sm text-gray-500">Financial instruments classification, impairment, and hedge accounting</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            {classificationData.map((item, idx) => (
                                <div key={idx}>
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                                        <div
                                            className={`h-1.5 rounded-full ${idx === 0 ? 'bg-blue-500' : idx === 1 ? 'bg-green-500' : 'bg-orange-500'}`}
                                            style={{ width: `${item.percentage}%` }}
                                        />
                                    </div>
                                    <p className="text-xs text-gray-400 mt-0.5">{item.percentage}% of total</p>
                                </div>
                            ))}
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total Financial Instruments</span>
                                    <span className="font-bold text-blue-600">{formatCurrency(classificationData.reduce((sum, item) => sum + item.amount, 0))}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="border-l-4 border-l-yellow-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Impairment (ECL)</h4>
                        <div className="space-y-3">
                            {eclData.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center">
                                    <div>
                                        <span className="text-sm text-gray-600">{item.stage}</span>
                                        <p className="text-xs text-gray-400">{item.description}</p>
                                    </div>
                                    <span className={`font-semibold ${idx === 0 ? 'text-green-600' : idx === 1 ? 'text-yellow-600' : 'text-red-600'}`}>
                            ...
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total ECL</span>
                                    <span className="font-bold text-red-600">{formatCurrency(eclData.reduce((sum, item) => sum + item.amount, 0))}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card className="border-l-4 border-l-purple-500">
                    <CardContent className="p-4">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Hedge Accounting</h4>
                        <div className="space-y-3">
                            {hedgeData.map((item, idx) => (
                                <div key={idx}>
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600">{item.type}</span>
                                        <span className="font-semibold text-gray-900">{formatCurrency(item.amount)}</span>
                                    </div>
                                    <p className="text-xs text-gray-400 ml-0">{item.description}</p>
                                </div>
                            ))}
                            <div className="pt-2 border-t border-gray-200">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-gray-700">Total Hedges</span>
                                    <span className="font-bold text-purple-600">{formatCurrency(hedgeData.reduce((sum, item) => sum + item.amount, 0))}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-wrap gap-4">
                <div className="flex-1 min-w-[200px] relative">
                    ...
                    <Input
                        placeholder="Search IFRS 9 reports..."
                        value={searchTerm}
                            ...
                            <tr>
                                <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <p className="font-medium">No IFRS 9 reports found</p>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            IFRS 9 Report Details
                        </DialogTitle>
                        <DialogDescription>
                            Detailed view of IFRS 9 - Financial Instruments report
                        </DialogDescription>
                    </DialogHeader>
                    {selectedReport && (
            ...
            <Dialog open={isGenerateModalOpen} onOpenChange={setIsGenerateModalOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Configure and generate a new IFRS 9 - Financial Instruments report
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
                ...
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Export IFRS 9 Reports
                        </DialogTitle>
                        <DialogDescription>
                            Export IFRS 9 reports in your preferred format.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
