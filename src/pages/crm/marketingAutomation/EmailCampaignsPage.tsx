// src/pages/crm/marketingAutomation/EmailCampaignsPage.tsx
...
import type { EmailCampaign } from '../../../types/crm/crm.types';
...
const EmailCampaignsPage: React.FC = () => {
    ...
    const handleSend = async (campaign: EmailCampaign) => {
        try {
            ...
            showToast.success('Email campaign sent successfully');
            fetchCampaigns();
        } catch (error) {
            showToast.error('Failed to send email campaign');
        } finally {
            setIsSending(false);
        }
    };
    ...
    const handleView = (campaign: EmailCampaign) => {
        navigate(`/crm/campaigns/email/${campaign.id}`);
    };

    const handleEdit = (campaign: EmailCampaign) => {
        navigate(`/crm/campaigns/email/edit/${campaign.id}`);
    };
        ...
        >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 flex items-center gap-2" onClick={() => navigate('/crm/campaigns/email/add')}>
                        <Plus size={16} />
                        New Campaign
                    </Button>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-wrap gap-4">
                ...
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                    ...
                    <SelectContent>
                        ...
                        {['Draft', 'Scheduled', 'Sending', 'Sent', 'Failed', 'Paused', 'Cancelled'].map((status) => (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                {paginatedCampaigns.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <Button className="mt-4 bg-indigo-600 hover:bg-indigo-700" onClick={() => navigate('/crm/campaigns/email/add')}>
                        ...
                        <table className="w-full">
                            ...
                            {paginatedCampaigns.map((campaign) => (
                                <tr key={campaign.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => handleView(campaign)}>
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-600 max-w-xs truncate">{campaign.subject}</td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3 text-right font-medium">{campaign.openCount || 0}</td>
                                    <td className="px-4 py-3 text-right font-medium">{campaign.clickCount || 0}</td>
            ...
            <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Are you sure you want to delete this email campaign? This action cannot be undone.</DialogDescription>
                    </DialogHeader>
                    {selectedCampaign && (
                        <div className="py-4">
            ...
            <Dialog open={isDuplicateModalOpen} onOpenChange={setIsDuplicateModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Create a copy of this email campaign with all its settings.</DialogDescription>
                    </DialogHeader>
                    {selectedCampaign && (
                        <div className="py-4">
                            <div className="p-3 bg-gray-50 rounded-lg">
